...
export function searchLocalContent(term: string): SearchHit[] {
  ...
  if (/garage|temp|thermal|trace|freeze|flood|leak|iot|dht|probe|sensor|esp32|pico|alert|ingest/.test(q)) {
    extras.push({
      ...
      title: 'ThermalTrace',
      description:
        'Open-source garage freeze and flood monitoring — ESP32/Pico ingest, time-to-freeze clock, Home Assistant HACS, Free/Member/Pro/Portfolio at thermaltrace.dev.',
